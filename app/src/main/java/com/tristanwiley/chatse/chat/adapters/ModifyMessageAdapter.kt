...
class ModifyMessageAdapter(val dialogMessages: MutableList<String>, context: Context) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View? {
        val viewHolder: com.tristanwiley.chatse.chat.adapters.ModifyMessageAdapter.ViewHolder
        var view = convertView

        if (view == null) {
            view = layoutInflater.inflate(android.R.layout.simple_list_item_1, parent, false)

            viewHolder = com.tristanwiley.chatse.chat.adapters.ModifyMessageAdapter.ViewHolder()
            viewHolder.textView = view.findViewById<TextView>(android.R.id.text1)
            view.tag = viewHolder
        } else {
            viewHolder = view.tag as com.tristanwiley.chatse.chat.adapters.ModifyMessageAdapter.ViewHolder
        }
